...
export const RoleSwitch = ({ currentRole, onChange, disabled }: RoleSwitchProps) => {
  return (
    <div className="flex items-center space-x-2 bg-gray-800/50 p-2 rounded-lg backdrop-blur-sm">
      <User className={`w-4 h-4 ${currentRole === 'student' ? 'text-yellow-400' : 'text-gray-400'}`} />
      <Switch
      ...
      />
      <ChalkboardTeacher className={`w-4 h-4 ${currentRole === 'teacher' ? 'text-yellow-400' : 'text-gray-400'}`} />
